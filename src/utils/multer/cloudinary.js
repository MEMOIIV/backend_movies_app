...
export const deleteResources = async ({
  ...
  return await cloud().api.delete_resources(public_id, options);
};
...
// export const deleteSubFolder = async ({ prefix = "" } = {}) => {
//   const { folders } = await cloud().api.sub_folders(`${process.env.APPLICATION_NAME}/${prefix}`);
//   for (const folder of folders) {
//     await cloud().api.delete_folder(folder.path);
//   }
// };

// export const deleteFolder = async ({ prefix = "" } = {}) => {
//   return await cloud().api.delete_folder(`${process.env.APPLICATION_NAME}/${prefix}`);
// };
